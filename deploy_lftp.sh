...
lftp -u ${FTP_USER},"${FTP_PASS}" ftp://${FTP_HOST} -e "set ftp:ssl-allow no; mirror -R -v --ignore-time -x '\.env' -x '.*\.py$' -x '.*\.sh$' -x '\.git' /Users/mubarak/dev/ro_naturalis/web_app/portfolio_landing/ /public_html/; quit"
